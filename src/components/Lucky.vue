<template>
	<div class="lucky">
		<canvas class="can" ref="can" width="414" height="896"></canvas>
		<canvas ref="heartCan" width=50 height=80 style="display:none;"></canvas>
	</div>	
</template>
...
export default {
	...
	mounted() {
		...
		drawHeart(heartCtx, 25,25,10, 0, '#FFC0CB');
		...
		(async () => {
			await startDrawTextAndHeart(container, heartCan, 200, 110, '小可爱');
			await startDrawTextAndHeart(container, heartCan, 300, 40, '我超喜欢你的');
			await startDrawTextAndHeart(container, heartCan, 400, 40, '做我女朋友吧');
		})();
